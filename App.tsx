...
import { User, ClassModel, UserRole } from './types';
import { Plus, Users, Copy, Check, Terminal, Shield } from 'lucide-react';
...
export default function App() {
  ...
  if (selectedClass) {
    return (
      <Layout user={user} onLogout={() => setUser(null)} activeTab="class" onTabChange={() => setSelectedClass(null)}>
              ...
              >
                ...
                <div className="p-6">
                  ...
                  <div className="mt-6 pt-4 border-t border-slate-800 flex justify-between items-center text-xs text-slate-500 font-mono uppercase tracking-wider">
                    <span>INIT: {new Date(cls.created_at).toLocaleDateString()}</span>
         ...
         <div className="bg-slate-900/50 backdrop-blur rounded-2xl shadow-lg border border-slate-800 p-8 max-w-2xl animate-in slide-in-from-right duration-500">
            ...
            <div className="space-y-6">
               ...
               {user.role === UserRole.STUDENT && (
                   <div className="bg-slate-950/50 p-4 rounded-xl border border-slate-800">
                     <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Access Code</label>
